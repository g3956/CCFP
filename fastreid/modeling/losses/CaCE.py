...
def cross_entropy_loss_ca(pred_class_outputs, gt_classes, eps, class2cid, alpha=0.2):
    num_classes = pred_class_outputs.size(1)

    mask = []
    for gt in gt_classes:
        temp = class2cid[int(gt)]
        nega_one = -torch.ones_like(temp)
        temp = torch.where(temp==0, nega_one, temp)
        mask.append(temp.unsqueeze(0))
